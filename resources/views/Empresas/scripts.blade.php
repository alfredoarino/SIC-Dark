...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        tabla = $('#tabla_empresas').dataTable( {
            'destroy': true,
            'processing': true,
            'responsive': true,
            'autoWidth': false,
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        $('#tabla_empresas tbody').on('click','.editar', function(){
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        $('#tabla_empresas tbody').on('click','.eliminar', function(){
            //Obtenemos el id del registro
            let data = tabla.api().row($(this).parents()).data();
            swal({
    ...
    //*********************************************************/
    function editar(id){
            let url_ = "{{ route('empresas.buscarRegistro') }}"
            $.ajax({
                data:  {"id":id},
                url: url_,
                type:  'get',
    ...
    function eliminar(id){
            let url_ = "{{ route('empresas.eliminar') }}"
    ...
    function grabaRegistro(){
        ...
        let url;
        if ($('#id_').val()===''){
            url = "{{ route('empresas.nuevo') }}"
        }
        else {
            url = "{{ route('empresas.actualizar') }}"
        }

        //OBTENEMOS LOS DATOS CON UN FORMDATA
    ...
    function validaCamposModal(){
        ...
        //NOMBRE (obligatorio)
        if ($('#nombre').val()==''){
            toastr.error('Se tiene que introducir el nombre de la empresa');
            $('#nombre').focus();
            ret = false;
        }
        //CIF (obligatorio)
        if ($('#cif').val()==''){
            toastr.error('Se tiene que introducir el CIF de la empresa');
            $('#cif').focus();
            ret = false;
        }
        //CÓDIGO (obligatorio)
        if ($('#codigo').val()==''){
            toastr.error('Se tiene que introducir el código de la empresa');
            $('#codigo').focus();
            ret = false;
        }
