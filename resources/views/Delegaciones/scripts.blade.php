...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        tabla = $('#tabla_delegaciones').dataTable( {
            'destroy': true,
            'processing': true,
            'responsive': true,
            'autoWidth': false,
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        $('#tabla_delegaciones tbody').on('click','.editar', function(){
        ...
        >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
        $('#tabla_delegaciones tbody').on('click','.eliminar', function(){
            //Obtenemos el id del registro
            let data = tabla.api().row($(this).parents()).data();
            swal({
                title: 'Estás seguro/a?',
                text: "Se eliminará la delegación!",
                type: 'warning',
    ...
    //*********************************************************/
    function editar(id){
            let url_ = "{{ route('delegaciones.buscarRegistro') }}"
            $.ajax({
                data:  {"id":id},
                url: url_,
                type:  'get',
    ...
    function eliminar(id){
            let url_ = "{{ route('delegaciones.eliminar') }}"
    ...
    function grabaRegistro(){
        ...
        let url;
        if ($('#id_').val()===''){
            url = "{{ route('delegaciones.nuevo') }}"
        }
        else {
            url = "{{ route('delegaciones.actualizar') }}"
        }
    ...
    function cargaDatos(response){
        $('#id_').val(response['id']);
        $('#titulo').html(response['siglas']+' - '+response['nombre']);
    ...
    function validaCamposModal(){
        ...
        //NOMBRE (obligatorio)
        if ($('#nombre').val()==''){
            toastr.error('Se tiene que introducir el nombre de la delegación');
            $('#nombre').focus();
            ret = false;
        }
        //SIGLAS (obligatorio)
        if ($('#siglas').val()==''){
            toastr.error('Se tiene que introducir las siglas de la delegación');
            $('#siglas').focus();
            ret = false;
        }
